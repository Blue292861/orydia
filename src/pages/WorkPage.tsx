...
const WorkPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!authorSlug || !titleSlug || books.length === 0) return;
  ...
  const handleStartReading = () => {
    ...
    // Navigate back to main app and trigger the work selection
    navigate('/', { 
      state: { 
  ...
  // Redirect if work not found
  if (authorSlug && titleSlug && books.length > 0 && audiobooks.length > 0 && !foundWork) {
    return <Navigate to="/" replace />;
  }

  // Show loading if still searching
