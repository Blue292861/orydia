...
export const startReadingEpubChapter = async (bookId: string, chapterId: string): Promise<void> => {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('User not authenticated');
...
export const markEpubChapterCompleted = async (chapterId: string, bookId: string): Promise<void> => {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('User not authenticated');
...
export const removeEpubBookFromProgress = async (bookId: string): Promise<void> => {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('User not authenticated');
