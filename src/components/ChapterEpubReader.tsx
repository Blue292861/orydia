...
export const ChapterEpubReader: React.FC = () => {
  ...
  const previousChapterIdRef = useRef<string>('');
  ...
  useEffect(() => {
    console.log("🚀 INIT start", chapter?.id, loading);  // TEST
    ...
    if (previousChapterIdRef.current === chapter.id) {
      console.log('🔄 Same chapter, skipping re-init');
      return;
    }
    ...
    const initEpub = async () => {
      ...
      try {
        ...
        if (epubPreloadService.isCached(chapter.id)) {
          console.log('🚀 FORCE DIRECT URL', chapter.epub_url);
        ...
        // 🌐 SUPABASE CORS FIX
        console.log('🚀 DIRECT Supabase URL:', epubUrl);
        ...
        setTimeout(() => {
          ...
          // sandbox actuel
          const sb = iframe.sandbox;
          if (!sb.contains('allow-scripts') || !sb.contains('allow-same-origin')) {
            iframe.setAttribute('sandbox', 'allow-scripts allow-forms allow-pointer-lock');
            console.log('🔓 Sandbox fixé');
          }
    ...
    return () => {
      console.log("💥 CLEANUP", chapter?.id);
      cancelled = true;

      localStorage.removeItem(`cfi_${chapter?.id}`);
      ...
      try { if (renditionRef.current?.destroy) renditionRef.current.destroy(); } catch { }
      renditionRef.current = null;
      try { if (bookRef.current?.destroy) bookRef.current.destroy(); } catch { }
      bookRef.current = null;
      if (epubRootRef.current) epubRootRef.current.innerHTML = '';
  ...
  const handleMarkLastChapterComplete = async () => {
    ...
    // Navigué chap suivant
    const nextCh = allChapters.find(ch => ch.chapter_number === (chapter?.chapter_number || 0) + 1);
    if (nextCh?.id) {
      setTimeout(() => navigate(`/book/${bookId}/chapter/${nextCh.id}`), 300);
      return;
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden pb-16 md:pb-14">
        {epubError && (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            ...
            <Button onClick={() => navigate(0)}>Recharger</Button>
          </div>
        )}
        <>
      ...
      <ChapterCompletionAnimation
        ...
        onContinue={() => {
          const nextId = pendingNextChapterId;
